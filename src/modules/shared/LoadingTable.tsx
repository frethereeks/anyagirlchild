...
export default function LoadingTable({ image, slug, title }: TBlogItemProp) {
    return (
            ...
            <figure className="relative rounded-lg overflow-hidden h-36 sm:h-40 md:h-40 bg-primary">
                {
                    image ?
                        ...
                        <Image src={ASSET_URL["back_to_school"]} alt={title} className="absolute left-0 top-0 w-full h-full object-cover group-hover:-rotate-2 group-hover:scale-110 group-hover:grayscale-[50%]" fill />
                }
            </figure>
