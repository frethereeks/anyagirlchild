...
export default function BlogCard({ image, slug, title }: TBlogItemProp) {
    return (
            ...
            <figure className="relative rounded-lg overflow-hidden h-36 sm:h-40 md:h-40 bg-primary">
                {
                    ...
                    <Image src={ASSET_URL["back_to_school"]} alt={title} className="absolute left-0 top-0 w-full h-full object-cover group-hover:scale-110 group-hover:grayscale-[50%]" fill />
                }
            </figure>
            <div className="flex-1 flex flex-col gap-2 px-2">
                {/* <p className="text-[.55rem] sm:text-[.65rem] md:text-[.7rem] text-danger font-normal uppercase tracking-wider">{category}</p> */}
                <h3 className="text-sm sm:text-lg lg:text-xl text-text font-medium font-grotesk mb-5 leading-none truncate line-clamp-2 whitespace-pre-wrap">{title}</h3>
