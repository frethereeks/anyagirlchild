    ...
    <header ref={headerRef} className={`${fixed ? "fixed backdrop-blur-sm" : "sticky"} flex flex-col py-2 px-4 w-full top-0 left-0 z-[990] ${fixed || navShow ? 'bg-white shadow-md shadow-primary/10' : 'bg-transparent'}`}>
      <div className="container w-full mx-auto flex flex-row gap-2 sm:gap-3 items-center sm:justify-between">
        ...
        <nav className={`absolute z-[990] md:static flex-1 flex flex-col md:flex-row md:items-center top-full shadow-md transition-all duration-300 ${navShow ? 'left-0' : 'left-[100vw]'} w-screen md:w-max md:shadow-none bg-white md:bg-transparent mx-0`}>
            ...
            {headerLinks.map((item) => (
              <li key={item.id} className={`relative group`}>
                <div className={`relative flex items-center w-full cursor-pointer hover:bg-primary hover:text-slate-100 md:hover:text-slate-700 md:hover:bg-transparent`}>
                  <Link href={item.link} className={`overflow-hidden relative md:w-max hover:before:transiton-all before:md:duration-300 before:md:absolute before:md:bottom-0 before:md:h-[2px] before:md:-rounded[2rem] before:md:bg-secondary before:md:w-0 before:md:-translate-x-1/2   ${item.link === location ? 'text-slate-700 md:before:left-1/2 md:before:w-[30%]' : 'hover:md:before:left-1/2 hover:md:before:w-[30%] text-slate-700 border-b-white'} ${fixed ? 'text-danger' : 'text-slate-700 border-b-white'} md:bg-transparent py-2 px-5 md:px-4 hover:bg-primary hover:text-slate-100 md:hover:text-slate-700 md:hover:bg-transparent text-sm font-semibold`}>{item.title}</Link>
          ...
          <Link href={appRoutePaths.donation} className="relative flex gap-2 items-center self-center md:rounded-md w-full md:w-max md:ml-auto px-5 md:px-7 py-2 h-max bg-secondary text-white text-sm font-semibold">Donate</Link>
        </nav>
